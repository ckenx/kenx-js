...
.command({
  ...
  handler: async argv => {
    const 
    packageJsonData = JSON.parse( await fs.readFile( path.join( rootDir, 'package.json' ) ) ),
    packageDirs = ( await Promise.all( packageJsonData.workspaces.map( item => glob( item ) )) ).flat()
